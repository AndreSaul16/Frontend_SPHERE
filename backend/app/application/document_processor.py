...
async def process_document(
    ...
    try:
        logger.info(f"Procesando documento: {filename} para agente {agent_id} (user: {user_id})")
        ...
        # 4. Store (con user_id para aislamiento multi-tenant)
        stored = await store_document_vectors(agent_id, file_id, filename, chunks, embeddings, user_id)
        ...
        from app.infrastructure.database import get_custom_agents_collection
