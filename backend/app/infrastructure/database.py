...
class Database:
    ...
    def __init__(self):
        ...
        if MONGO_URI and (
            "mongodb+srv" in MONGO_URI or "tls=true" in MONGO_URI.lower()
        ):
    ...
    def connect(self) -> bool:
        ...
        if self._connected:
            ...
            if (
                current_loop
                and self.client
                and self.client.get_io_loop() != current_loop
            ):
                logger.warning(
                    "🔄 Detectado cambio de Event Loop, reconectando cliente async..."
                )
                if self.client:
                    self.client.close()
...
# Nuevas colecciones multi-tenant
def get_users_collection():
    """Colección para perfiles de usuario."""
    return db.get_async_db()["users"]


def get_oauth_credentials_collection():
    """Colección para credenciales OAuth cifradas."""
    return db.get_async_db()["oauth_credentials"]


def get_oauth_states_collection():
    """Colección para estados OAuth efímeros (CSRF)."""
    return db.get_async_db()["oauth_states"]


def get_contacts_collection():
    """Colección para whitelist de contactos por usuario."""
    return db.get_async_db()["contacts"]


def get_user_agent_overrides_collection():
    """Colección para overrides de agentes por usuario."""
    return db.get_async_db()["user_agent_overrides"]


def get_idempotency_keys_collection():
    """Colección para idempotency keys (TTL 24h)."""
    return db.get_async_db()["idempotency_keys"]


def get_tool_audit_log_collection():
    """Colección para auditoría de tool executions."""
    return db.get_async_db()["tool_audit_log"]


def get_service_credentials_collection():
    """Colección para credenciales de servicios externos cifradas (API keys, tokens)."""
